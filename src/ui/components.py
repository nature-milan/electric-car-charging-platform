...
from state.session_state import (
    set_manual_charge_duration_minutes,
    stop_charging_button_event,
)
...
def render_controls(
    *,
    ...
    with c2:
        stop_charging_button_event()
        simulated_datetime = st.session_state["simulated_datetime"]
        ...
        if st.button(
            ...
            if charge_is_override and is_within_schedule_window(simulated_datetime):
                handle_stop_charge()  # stop manual charge only
        ...
        if st.session_state["show_stop_confirm"]:
            st.warning(
                "Manual charging has stopped. Do you also want to stop scheduled charging?"
            )
            ...
            with col1:
                if st.button(
                    "Yes, stop scheduled charging", key="confirm_stop_schedule_yes"
                ):
                    handle_stop_charge()  # now this will pause scheduled charging
                    st.session_state["show_stop_confirm"] = False
                    st.rerun()

            with col2:
                if st.button(
                    "No, keep scheduled charging", key="confirm_stop_schedule_no"
                ):
                    st.session_state["show_stop_confirm"] = False
                    st.rerun()
